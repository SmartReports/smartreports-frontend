  ...
  <v-container>
    ...
    <v-container>
      <v-list>
        ...
        <v-divider></v-divider>
        <v-container>
          <v-list-item v-for="report in archived_reportsAsItem" :key="report.id">
            <v-row>
              <v-col cols="3" md="3" sm="3">
                <v-card-subtitle class="text-h6">{{ report.name }}</v-card-subtitle>
              </v-col>
              <v-col cols="3" md="3" sm="3">
                <v-card-subtitle class="text-h6">{{ report.created }}</v-card-subtitle>
              </v-col>
              <v-col cols="3" md="3" sm="3">
                <v-card-subtitle class="text-h6">{{ report.frequency }}</v-card-subtitle>
              </v-col>
              <v-col cols="3" md="3" sm="3">
                <v-btn @click="downloadPDF(report.file, report.file_name)" height="30" width="90%"><v-icon>mdi-download</v-icon></v-btn>
              </v-col>
              <v-divider></v-divider>
            </v-row>
          </v-list-item>
  ...
  computed:{
      ...
      archived_reportsAsItem() {
        return this.archived_reports.map((report: any) =>  ({
          ...
          name: this.mainStore.user_reports.filter((r) => r.id == report.template)[0].name,
          frequency: this.mainStore.user_reports.filter((r) => r.id == report.template)[0].frequency,
        })) as any[];
      },
  },
  methods: {
